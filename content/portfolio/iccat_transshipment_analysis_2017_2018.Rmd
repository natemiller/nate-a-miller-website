...
outline all the matches here [Encounters] and summarize the by vessel and voyage [here](Encounters Matched by Observered Transshipment)
...
[here](Loitering Events) and again summarize the matching by vessel and voyage [here](Loitering Events Matched to Observered Transshipment)
...
We still need to explore why this is, though there are some possibilities. With [TAISEI MARU NO 24](TAISEI MARU NO 24) and
...
iccat_obs_data_2017_2018 <- readr::read_csv('./ICCAT_transshipments_2017_2018 - ICCAT_transshipments_2017_2018..csv')
```
...
shp_list <- list(
  land_sf = "../../shapefiles/ne_10m_admin_0_countries/ne_10m_admin_0_countries_polygons/ne_10m_admin_0_countries.shp",
  eez_sf = "../../shapefiles/World_EEZ_v8_20140228_LR/World_EEZ_v8_2014.shp"
)
...
##### Encounters
```{r}
DT::datatable(encounters_match_tot)
```


##### Loitering Events
```{r}
DT::datatable(loiter_match_tot)
```
...
DT::datatable(loitering_match_counts %>%
  inner_join(events, by = c('column_label' = 'column_label')) %>%
  mutate(frac_match = matches/n)) # %>%
  #summarise(min(frac_match))
...
DT::datatable(matched_encounters %>%
  inner_join(tot_encounters, by = "column_label") %>%
...
DT::datatable(loitering_match_counts %>%
  inner_join(total_loitering_events, by = c('column_label' = 'column_label')) %>%
  mutate(frac_match = matches/n))
```
